...
export const usersIdSeqInCredentials = credentials.sequence("users_id_seq", {
	startWith: "1",
	increment: "1",
	minValue: "1",
	maxValue: "2147483647",
...
export const relationSingerIdSeq = pgSequence("relation_singer_id_seq", {
	startWith: "1",
	increment: "1",
	minValue: "1",
	maxValue: "2147483647",
...
export const relationsProducerIdSeq = pgSequence("relations_producer_id_seq", {
	startWith: "1",
	increment: "1",
	minValue: "1",
	maxValue: "2147483647",
...
export const usersInCredentials = credentials.table(
	"users",
	{
		id: integer()
			.default(sql`nextval('credentials.users_id_seq'::regclass)`)
			.notNull(),
		nickname: text(),
...
export const loginSessionsInCredentials = credentials.table(
	"login_sessions",
	{
		id: text().notNull(),
		uid: integer().notNull(),
	...
	(table) => [
		...
		uniqueIndex("login_sessions_pkey").using(
			"btree",
...
export const humanClassifiedLables = pgTable(
	...
	(table) => [
		index("idx_classified-labels-human_aid").using(
			"btree",
			table.aid.asc().nullsLast().op("int8_ops")
		),
		index("idx_classified-labels-human_author").using(
			"btree",
			table.uid.asc().nullsLast().op("int4_ops")
		),
		index("idx_classified-labels-human_created-at").using(
			"btree",
			table.createdAt.asc().nullsLast().op("timestamptz_ops")
		),
		index("idx_classified-labels-human_label").using(
			"btree",
...
export const videoSnapshot = pgTable(
	...
	(table) => [
		...
		index("idx_vid_snapshot_time").using(
			"btree",
			table.createdAt.asc().nullsLast().op("timestamptz_ops")
		),
...
export const labellingResult = pgTable(
	...
	(table) => [
		...
		index("idx_labeling_model-version").using(
			"btree",
		...
		uniqueIndex("labeling_result_pkey").using(
			"btree",
...
export const latestVideoSnapshot = pgTable(
	...
	(table) => [
		index("idx_latest-video-snapshot_time").using(
			"btree",
			table.time.asc().nullsLast().op("timestamptz_ops")
		),
		index("idx_latest-video-snapshot_views").using(
			"btree",
...
export const relationsProducer = pgTable(
	"relations_producer",
	{
		id: integer()
			.default(sql`nextval('relations_producer_id_seq'::regclass)`)
			.primaryKey()
			.notNull(),
		// You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const eta = pgTable(
	...
	(table) => [
		index("idx_eta_eta_current_views").using(
			"btree",
			table.eta.asc().nullsLast().op("int4_ops"),
			table.currentViews.asc().nullsLast().op("int4_ops")
		),
...
export const songs = pgTable(
	...
	(table) => [
		...
		uniqueIndex("unq_songs_netease_id").using(
			"btree",
			table.neteaseId.asc().nullsLast().op("int8_ops")
		),
...
export const snapshotSchedule = pgTable(
	...
	(table) => [
		index("idx_snapshot_schedule_aid_status_type").using(
			"btree",
			table.aid.asc().nullsLast().op("int8_ops"),
			table.status.asc().nullsLast().op("text_ops"),
			table.type.asc().nullsLast().op("text_ops")
		),
		index("idx_snapshot_schedule_status_started_at").using(
			"btree",
			table.status.asc().nullsLast().op("timestamptz_ops"),
			table.startedAt.asc().nullsLast().op("text_ops")
		),
		uniqueIndex("snapshot_schedule_pkey").using(
			"btree",
...
export const relationSinger = pgTable(
	"relation_singer",
	{
		id: integer()
			.default(sql`nextval('relation_singer_id_seq'::regclass)`)
			.primaryKey()
			.notNull(),
		// You can use { mode: "bigint" } if numbers are exceeding js number limitations
		songId: bigint("song_id", { mode: "number" }).notNull(),
...
export const videoTypeLabelInInternal = internal.table(
	"video_type_label",
	{
		id: serial().primaryKey().notNull(),
		// You can use { mode: "bigint" } if numbers are exceeding js number limitations
