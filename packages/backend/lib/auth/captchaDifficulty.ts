...
export const getCaptchaDifficultyConfigByRoute = async (
	sql: Psql,
	route: string
): Promise<CaptchaDifficultyConfig[]> => {
...
export const getCurrentCaptchaDifficulty = async (sql: Psql, c: Context | string): Promise<number | null> => {
	...
	const maxDuration = configs.reduce((max, config) => Math.max(max, config.duration), 0);
